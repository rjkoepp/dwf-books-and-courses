---
title: Google Style Guide for Python
hide_title: false
sidebar_label: Google style guide
description: Details about Google's Python style guide
draft: false
...
Everything below appears on [Google's Python Style Guide](https://google.github.io/styleguide/pyguide.html).
...
To help you format code correctly, we've created a settings file for Vim:

```vim title="Google settings file"
...
If needed, global variables should be declared at the module level and made internal to the module by prepending an `_` to the name. External access to global variables must be done through public module-level functions. See [Naming](https://google.github.io/styleguide/pyguide.html#s3.16-naming) below.

While module-level constants are technically variables, they are permitted and encouraged. For example: `MAX_HOLY_HANDGRENADE_COUNT = 3`. Constants must be named using all caps with underscores. See [Naming](https://google.github.io/styleguide/pyguide.html#s3.16-naming) below.
...
- private [`TypeVar`s](https://google.github.io/styleguide/pyguide.html#typing-type-var) with no constraints (e.g. `_T`, `_U`, `_V`)
...
The Python type system has [generics](https://peps.python.org/pep-0484/#generics). The factory function `TypeVar` is a common way to use them.
